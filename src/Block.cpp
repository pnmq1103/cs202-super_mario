...
Block::Block(Vector2 pos, int w, int h, BlockType type, bool solid, int spriteId)
    : position(pos), width(w), height(h), type(type), solid(solid), spriteId(spriteId) {}
...
void Block::render(const Texture2D& texture, const std::unordered_map<int, Rectangle>& spriteRects) const {
