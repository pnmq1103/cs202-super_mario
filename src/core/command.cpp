...
void Command::InitializeInstructionsButton() {
  if (!instructions_initialized_) {
    instructions_button_ = Button(
      ...
      "res/sprites/object/objects.png");
...
void Command::DrawInstructionsPanel() {
  ...
  DrawText(
    "� Jump on enemies to defeat them", rightCol, startY + lineSpacing * 1, 12,
    BLACK);
  DrawText(
    "� Avoid touching enemies from the side", rightCol,
    startY + lineSpacing * 2, 12, BLACK);
  DrawText(
    "� Hit blocks from below to get items", rightCol, startY + lineSpacing * 3,
    12, BLACK);
  DrawText(
    "� Collect power-ups to grow stronger", rightCol, startY + lineSpacing * 4,
    12, BLACK);

  DrawText(
    "=== CHARACTER STATES ===", rightCol, startY + lineSpacing * 6, 16, MAROON);
  DrawText(
    "� Small: Default state", rightCol, startY + lineSpacing * 7, 12, BLACK);
  DrawText(
    "� Super: Can break blocks", rightCol, startY + lineSpacing * 8, 12, BLACK);
  DrawText(
    "� Fire: Shoots projectiles", rightCol, startY + lineSpacing * 9, 12,
    BLACK);
  DrawText(
    "� Star: Temporary invincibility", rightCol, startY + lineSpacing * 10, 12,
    BLACK);

  DrawText(
    "=== ENEMY TYPES ===", rightCol, startY + lineSpacing * 12, 16, PURPLE);
  DrawText(
    "� Goomba: Brown mushroom enemies", rightCol, startY + lineSpacing * 13, 12,
    BLACK);
  DrawText(
    "� Koopa: Green turtle enemies", rightCol, startY + lineSpacing * 14, 12,
    BLACK);
  DrawText(
    "� Piranha Plant: Dangerous pipe plants", rightCol,
    startY + lineSpacing * 15, 12, BLACK);
  DrawText(
    "� Bowser: Final boss with multiple hits", rightCol,
