...
void GameManaging::DrawBackground() {
    ...
    try {
        Texture backgroundTex = ResManager::GetBackground(backgroundType_ + 1);
...
void GameManaging::DrawBlock(const Block* block) {
    ...
    try {
        ...
        Texture tileTexture = ResManager::GetTile(block->GetSpriteId() + 1);
...
void GameManaging::DrawStats() const {
    ...
    DrawText(TextFormat("Time: %d", countdownSeconds_ - (int)gameTime_), 10, 100, 24, GREEN);
}
...
void GameManaging::CheckCollisions(Character* character) {
    ...
    if (CheckBlockCollision(characterRect, correction)) {
        ...
        // Character doesn't have SetPosition, so we'll use StopY for vertical corrections
        if (correction.y != 0) {
            if (correction.y < 0) {
...
bool GameManaging::CheckBlockCollision(Rectangle characterRect, Vector2& correction) {
    bool collided = false;
    correction = {0, 0};
    ...
    for (auto& block : blocks_) {
        if (block->IsSolid() && block->CheckCollision(characterRect)) {
            ...
            float overlapX = std::min(characterRect.x + characterRect.width - blockRect.x,
                                    blockRect.x + blockRect.width - characterRect.x);
            float overlapY = std::min(characterRect.y + characterRect.height - blockRect.y,
...
void GameManaging::CreateBlockFromType(int tileType, Vector2 position) {
    ...
    switch (tileType) {
        case 1: // Solid block
            newBlock = std::make_unique<StaticBlock>(position, 32, 32, BlockType::Solid, tileType);
            break;
        case 2: // Question block
            newBlock = std::make_unique<QuestionBlock>(position, PowerUpType::Coin, tileType);
            break;
        case 3: // Brick block
            newBlock = std::make_unique<StaticBlock>(position, 32, 32, BlockType::Brick, tileType);
            break;
        default:
