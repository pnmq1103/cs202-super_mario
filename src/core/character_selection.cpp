...
SelectedCharacter CharacterSelectionScene::global_selected_character_ = SelectedCharacter::MARIO;
...
void CharacterSelectionScene::Init() {
  background_ = LoadTexture("res/character_selection.png");
...
void CharacterSelectionScene::Draw() {
  timer_ += GetFrameTime();
  float t = std::min(timer_ / duration_, 1.0f);
  float ease = 1 - powf(1 - t, 3);
  
  float bg_x = (constants::screenWidth - background_.width) / 2;
  float bg_y = (constants::screenHeight - background_.height) / 2;
  
  DrawTextureV(background_, {bg_x, bg_y}, RAYWHITE);
...
void CharacterSelectionScene::Resume() {
  timer_ = 0;
  last_input_ = GetTime(); 
  App.Media().PlayMusic("choose_character");
}

SceneType CharacterSelectionScene::Type() {
  return type_;
}

void CharacterSelectionScene::SelectMario() {
  selectedCharacter_ = SelectedCharacter::MARIO;
  global_selected_character_ = SelectedCharacter::MARIO;
  std::cout << "Mario selected!" << std::endl;
}

void CharacterSelectionScene::SelectLuigi() {
...
void CharacterSelectionScene::StartGame() {
  std::cout << "Starting game with " << (selectedCharacter_ == SelectedCharacter::MARIO ? "Mario" : "Luigi") << std::endl;
  App.AddScene(std::make_unique<GameScene>());
}

void CharacterSelectionScene::DrawCharacterOptions() {
  ...
  // Draw Mario option
  Color mario_color = (selectedCharacter_ == SelectedCharacter::MARIO) ? YELLOW : WHITE;
  float mario_scale = (selectedCharacter_ == SelectedCharacter::MARIO) ? 1.2f : 1.0f;
  
  // Draw Mario rectangle (placeholder for character sprite)
  Rectangle mario_rect = {
    mario_x - char_width * mario_scale / 2,
    char_y - char_height * mario_scale / 2,
    char_width * mario_scale,
  ...
  DrawTextEx(GetFontDefault(), mario_label, 
    {mario_x - mario_label_sz.x / 2, char_y + char_height * mario_scale / 2 + 20}, 
    30, 1, mario_color);
  
  Color luigi_color = (selectedCharacter_ == SelectedCharacter::LUIGI) ? YELLOW : WHITE;
  float luigi_scale = (selectedCharacter_ == SelectedCharacter::LUIGI) ? 1.2f : 1.0f;
  
  Rectangle luigi_rect = {
    luigi_x - char_width * luigi_scale / 2,
    char_y - char_height * luigi_scale / 2,
    char_width * luigi_scale,
  ...
  DrawTextEx(GetFontDefault(), luigi_label, 
    {luigi_x - luigi_label_sz.x / 2, char_y + char_height * luigi_scale / 2 + 20}, 
    30, 1, luigi_color);
  ...
  if (selectedCharacter_ == SelectedCharacter::MARIO) {
    ...
    DrawText("", (int)(mario_x - 10), (int)indicator_y, 40, ColorAlpha(YELLOW, alpha));
  } else {
    float alpha = 0.5f + 0.5f * wave;
    DrawText("", (int)(luigi_x - 10), (int)indicator_y, 40, ColorAlpha(YELLOW, alpha));
  }
}

void CharacterSelectionScene::DrawInstructions() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    DrawTextEx(GetFontDefault(), instructions[i], 
      {text_x, instructions_y + i * 30}, 20, 1, DARKGRAY);
  }
}
