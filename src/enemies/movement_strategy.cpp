...
JumpingMovement::JumpingMovement(float horizontalSpeed, float jumpForce, float jumpInterval)
    : horizontalSpeed_(horizontalSpeed), jumpForce_(jumpForce), jumpInterval_(jumpInterval),
      jumpTimer_(0.0f), verticalVelocity_(0.0f), onGround_(true), movingLeft_(true), gravity_(400.0f) {}
...
BossMovement::BossMovement(float baseSpeed)
    : currentPhase_(BossPhase::WALKING), phaseTimer_(0.0f), speed_(baseSpeed),
      ...
      verticalVelocity_(0.0f), onGround_(true), targetPosition_({0, 0}), attackCount_(0) {}
...
void BossMovement::UpdateJumpingPhase(Enemy* enemy, float deltaTime) {
    ...
    // Apply gravity
    verticalVelocity_ += 400.0f * deltaTime;
    velocity.y = verticalVelocity_;
...
PatrolMovement::PatrolMovement(Vector2 pointA, Vector2 pointB, float speed)
    : pointA_(pointA), pointB_(pointB), speed_(speed), tolerance_(5.0f), movingToB_(true) {
...
void PatrolMovement::Update(Enemy* enemy, float deltaTime) {
    ...
    Vector2 direction = {currentTarget_.x - position.x, currentTarget_.y - position.y};
...
FollowingMovement::FollowingMovement(Vector2* target, float speed, float detectionRange)
...
void FollowingMovement::Update(Enemy* enemy, float deltaTime) {
    if (!enemy || !targetPosition_) return;
    
    Vector2 position = enemy->GetPosition();
    Vector2 velocity = enemy->GetVelocity();
    
    // Calculate distance to target
    Vector2 direction = {targetPosition_->x - position.x, targetPosition_->y - position.y};
    float distance = sqrt(direction.x * direction.x + direction.y * direction.y);
