...
KoopaTroopa::KoopaTroopa(Vector2 pos, float Nscale)
    : Enemy(pos, Nscale, EnemyType::Koopa), koopa_state(KoopaState::Walking), shell_timer(0.0f) {
    ...
    velocity.x = 25.0f; // Start walking right
    facing_left = false;
    
    std::cout << "Koopa spawned with initial velocity: " << velocity.x << std::endl;
}
...
void KoopaTroopa::OnHit() {
    if (koopa_state == KoopaState::Walking) {
        ...
        std::cout << "Koopa shell kicked! Sliding at speed: " << velocity.x << std::endl;
...
void KoopaTroopa::Update() {
    ...
    if (koopa_state == KoopaState::Sliding) {
        ...
        if (auto* sliding_movement = dynamic_cast<SlidingMovement*>(GetMovementStrategy())) {
...
void KoopaTroopa::OnHitFromAbove() {
    try {
        ...
        std::cerr << "Error in KoopaTroopa::OnHitFromAbove(): " << e.what() << std::endl;
...
void KoopaTroopa::OnHitFromSide() {
    try {
        ...
        std::cerr << "Error in KoopaTroopa::OnHitFromSide(): " << e.what() << std::endl;
        // Fallback: just mark as dead
        alive = false;
        state = EnemyState::Dead;
        velocity = {0.0f, 0.0f};
    ...
    if (movement_strategy_) {
        switch (koopa_state) {
            case KoopaState::Walking:
                clone->SetMovementStrategy(new WalkingMovement(40.0f));
                break;
            case KoopaState::Sliding:
                clone->SetMovementStrategy(new SlidingMovement(200.0f, facing_left));
                break;
