...
void PiranhaPlant::StartEmerging() {
  ...
  if (
    auto *stationary
    = dynamic_cast<StationaryMovement *>(GetMovementStrategy())) {
...
void PiranhaPlant::OnHitFromAbove() {
  try {
    ...
    std::cout << "Piranha Plant touched from above - Mario should take damage!"
              << std::endl;
  } catch (const std::exception &e) {
    std::cerr << "Error in PiranhaPlant::OnHitFromAbove(): " << e.what()
              << std::endl;
...
void PiranhaPlant::OnHitFromSide() {
  try {
    ...
    std::cerr << "Error in PiranhaPlant::OnHitFromSide(): " << e.what()
              << std::endl;
