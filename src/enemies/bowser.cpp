...
Bowser::Bowser(Vector2 pos, float Nscale)
    : Enemy(pos, Nscale, EnemyType::Bowser), hp(5), fire_timer(0.0f), attack_cooldown(3.0f), rage_mode(false) {
    // Set up boss movement pattern
    SetMovementStrategy(new BossMovement(20.0f)); // Use boss-specific movement
    ...
    std::cout << "Bowser spawned with initial velocity: " << velocity.x << std::endl;
}
    ...
    if (player_position && IsPlayerInRange(detection_range)) {
        ...
        if (state != EnemyState::Stunned) {
            if (should_face_left && velocity.x > 0) {
                velocity.x = -abs(velocity.x); // Make velocity negative when facing left
            } else if (!should_face_left && velocity.x < 0) {
                velocity.x = abs(velocity.x); // Make velocity positive when facing right
            }
        }
        
        // Different attack patterns based on health
        float attack_frequency = rage_mode ? 1.5f : attack_cooldown; // Faster attacks in rage mode
...
void Bowser::ShootFireball() {
    ...
    std::cout << "Bowser shoots fireball at position: " << spawn_position.x << ", " << spawn_position.y << std::endl;
}
...
void Bowser::EnterRageMode() {
    ...
    if (auto* boss_movement = dynamic_cast<BossMovement*>(GetMovementStrategy())) {
...
Enemy* Bowser::Clone() const {
    ...
    if (movement_strategy_) {
        clone->SetMovementStrategy(new BossMovement(20.0f));
    }
    return clone;
}
