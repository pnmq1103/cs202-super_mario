...
int main() {
  ...
  // Define sprite rectangles for each enemy type (adjust these based on your
sprite sheet)
  // These are example coordinates - you'll need to adjust based on your actual
sprite sheet GOOMBA (small enemies - 16x15/16 pixels) enemySpriteRects[0] = {0,
188, 16, 15}; // From ID 21: x=0, y=188, w=16, h=15 enemySpriteRects[1] = {17,
187, 16, 16}; // From ID 22: x=17, y=187, w=16, h=16

  // KOOPA TROOPA (medium enemies - 16x22/24 pixels)
  enemySpriteRects[2] = {17, 241, 16, 22}; // From ID 27: x=17, y=241, w=16,
h=22 enemySpriteRects[3] = {34, 239, 16, 24}; // From ID 28: x=34, y=239, w=16,
h=24 enemySpriteRects[4] = {0, 205, 16, 16}; // Shell from ID 23: x=0, y=205,
w=16, h=16
  ...
  enemyManager->SpawnEnemy(EnemyType::Goomba, {200.0f, 870.0f});    // Ground
Goomba enemyManager->SpawnEnemy(EnemyType::Goomba, {400.0f, 870.0f});    //
Another Goomba enemyManager->SpawnEnemy(EnemyType::Koopa, {300.0f, 870.0f}); //
Ground Koopa enemyManager->SpawnEnemy(EnemyType::Koopa, {350.0f, 770.0f}); //
Platform Koopa enemyManager->SpawnEnemy(EnemyType::Piranha, {820.0f, 870.0f});
// Piranha in pipe enemyManager->SpawnEnemy(EnemyType::Piranha, {1220.0f,
870.0f});  // Another Piranha
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsKeyPressed(KEY_FOUR)) {
      testMode = 3;
      // Spawn formation
      enemyManager->SpawnPatrolGroup(EnemyType::Goomba, {150.0f, 870.0f},
{350.0f, 870.0f}, 3);
    }
    ...
    if (testMode == 1 && spawnTimer >= 2.0f) {
      ...
      EnemyType types[] = {EnemyType::Goomba, EnemyType::Koopa,
EnemyType::Piranha}; EnemyType randomType = types[rand() % 3]; float randomX =
200.0f + (rand() % 800); enemyManager->SpawnEnemy(randomType, {randomX,
870.0f}); spawnTimer = 0.0f;
    }
      ...
      for (Enemy* enemy : enemyManager->GetEnemies()) {
        if (enemy && enemy->IsAlive()) {
          ...
          if (useSprites) {
            ...
            int spriteIndex = 0;
            int animFrame = ((int)(animationTimer * 4)) % 2; // 4 fps animation,
2 frames
            ...
            // Draw the sprite
            DrawTexturePro(enemyTexture, sourceRect, destRect, {0, 0}, 0.0f,
tint);
          ...
          if (enemy->GetState() == EnemyState::Stunned) {
            DrawText("!", (int)rect.x + (int)rect.width - 8, (int)rect.y - 10,
12, YELLOW); } else if (enemy->GetState() == EnemyState::Attacking) {
            DrawText("*", (int)rect.x + (int)rect.width - 8, (int)rect.y - 10,
12, RED);
          }
...
"Luigi"; DrawText(TextFormat("Character: %s", activeCharName), 10, 10, 20,
WHITE);
...
10, 35, 16, WHITE);
    ...
    DrawText(TextFormat("Enemies Defeated: %d", enemiesDefeated), 10, 120, 16,
LIME);
    ...
    if (enemyManager) {
      ...
      DrawText(TextFormat("Enemies Alive: %zu/%zu", aliveEnemies, totalEnemies),
10, 145, 16, WHITE);
      ...
      if (enemyManager->HasBoss()) {
        ...
        if (boss) {
          ...
          DrawText(TextFormat("Boss HP: %.0f%%", healthPercentage * 100), 10,
165, 18, RED);
        }
      }
    }
    ...
    // Test mode indicator
    const char* modeNames[] = {"Normal", "Spawn Test", "Boss Test", "Formation
Test"}; DrawText(TextFormat("Mode: %s", modeNames[testMode]), 10, 190, 16,
ORANGE);

    // Rendering mode indicator
    DrawText(TextFormat("Rendering: %s", useSprites ? "SPRITES" : "RECTANGLES"),
10, 210, 16, PURPLE);

    // Controls help
    DrawText("=== ENEMY TEST CONTROLS ===", 10, 240, 14, LIME);
    DrawText("Arrows: Move | UP: Jump | TAB: Switch Character", 10, 260, 12,
WHITE); DrawText("SPACE: Toggle Sprites/Rectangles", 10, 275, 12, GRAY);
    DrawText("1-4: Test Modes | Q/W/E: Difficulty Easy/Normal/Hard", 10, 290,
12, WHITE); DrawText("P/R: Pause/Resume | C: Clear All | B: Boss Rage", 10, 305,
12, WHITE); DrawText("G: Evolve | H: Die | S: Star Power | A: Hit Blocks", 10,
320, 12, WHITE); DrawText("Shift: Fireball | F1: Debug Info", 10, 335, 12,
WHITE);

    // Instructions
    DrawText("Jump on enemies to defeat them!", GetScreenWidth() - 280, 10, 16,
LIME); DrawText("Avoid touching from the side!", GetScreenWidth() - 280, 30, 14,
RED); DrawText("Use Star Power for invincibility!", GetScreenWidth() - 280, 50,
14, GOLD); DrawText( "Press SPACE to toggle sprites!", GetScreenWidth() - 280,
70, 14, GRAY);

    // Debug info
    if (showDebugInfo) {
      DrawText("=== DEBUG INFO ===", GetScreenWidth() - 200, 100, 12, ORANGE);
      DrawText(TextFormat("FPS: %d", GetFPS()), GetScreenWidth() - 200, 115, 10,
WHITE); DrawText(TextFormat("Delta: %.3f", deltaTime), GetScreenWidth() - 200,
130, 10, WHITE); DrawText(TextFormat("Camera: %.0f,%.0f", camera.target.x,
camera.target.y), GetScreenWidth() - 200, 145, 10, WHITE);
      DrawText(TextFormat("AnimTimer: %.2f", animationTimer), GetScreenWidth() -
200, 160, 10, WHITE); if (activeCharacter) { Rectangle rect =
activeCharacter->GetRectangle(); DrawText(TextFormat("Player: %.0f,%.0f",
rect.x, rect.y), GetScreenWidth() - 200, 175, 10, WHITE); Vector2 speed =
activeCharacter->GetSpeed(); DrawText(TextFormat("Speed: %.0f,%.0f", speed.x,
speed.y), GetScreenWidth() - 200, 190, 10, WHITE);
      }
    }
